...
const getMetadata = async (mint: anchor.web3.PublicKey): Promise<anchor.web3.PublicKey> => {
  ...
  export const unstakeNFT = async (wallet: NodeWallet, connection: Connection, mint: anchor.web3.PublicKey) => {
    const provider = new AnchorProvider(connection, wallet, {});
  const program = new Program<CherryServersMarketplace>(
    CherryServerIDL as unknown as CherryServersMarketplace,
    provider,
  );
  const stake = anchor.web3.PublicKey.findProgramAddressSync([Buffer.from("stake"), wallet.publicKey.toBytes(), mint.toBytes()], program.programId)[0];
    ...
    const vault = getAssociatedTokenAddressSync(mint, stake, true);
    const ix = await await program.methods
    .unstakeNft()
    .accountsPartial({
      user: wallet.publicKey,
      userNftAccount: ownerAta,
      mint,
      vaultNftAccount: vault,
      stakeAccount: stake,
