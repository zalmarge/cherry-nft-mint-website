...
import { COLLECTION_ADDRESS, TREASURY_ADDRESS } from "@/globals";
...
const getMetadata = async (mint: anchor.web3.PublicKey): Promise<anchor.web3.PublicKey> => {
  ...
  export const purchaseNFT = async (wallet: NodeWallet, connection: Connection, mint: anchor.web3.PublicKey, seller: anchor.web3.PublicKey) => {
  ...
  const listing = anchor.web3.PublicKey.findProgramAddressSync([Buffer.from("listing"), mint.toBytes()], program.programId)[0];
    ...
    .accountsPartial({
      ...
      collection: collection,
        listing,
        treasury: new anchor.web3.PublicKey(TREASURY_ADDRESS),
